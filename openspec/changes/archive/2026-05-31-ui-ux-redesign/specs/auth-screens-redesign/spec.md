...
Redesign `login_screen.dart` and `register_screen.dart` to use the PlayVerse visual identity established by the design system.
...
- The header must contain: a centred `SvgPicture.asset('assets/images/board.svg')` at 80×80, a `Text('PlayVerse')` in `AppTextStyles.displayLarge` (white), and a tagline `Text('Cờ Gomoku Online')` in `AppTextStyles.bodyMedium` (white 70% opacity).
